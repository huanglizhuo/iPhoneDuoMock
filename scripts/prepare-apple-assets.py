...
# The complete lock screens are captured from Apple's WallpaperRenderer after
# its geometry, shaders and depth-aware clock composition have rendered.
# Never concatenate cropped wallpaper textures: their silhouettes live in the
# scene geometry, not in the texture alpha channel.
from PIL import ImageOps

demo = pathlib.Path('public/demo/apple')
demo.mkdir(parents=True, exist_ok=True)
for name, width, height, plate in [
    ('inner', 2853, 2007, 'inner'),
    ('inner-portrait', 2007, 2853, 'outer'),
    ('outer', 1398, 2034, 'outer'),
    ('outer-landscape', 2034, 1398, 'inner'),
]:
    image = Image.open(source / f'lockscreen-{plate}-render.png').convert('RGB')
