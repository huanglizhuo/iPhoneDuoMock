...
截图在本地处理，不会上传。网页模拟支持允许嵌入的网站，导出时需授权捕获当前标签页。对于无法嵌入的网站，本地版可检查限制并生成静态截图，随后按截图模式编辑和导出。视频格式取决于浏览器支持情况。
...
如需自动网页截图，先运行 `npx playwright install chromium`。在网页模拟中提交网址后，点击「生成网页截图」；截图使用独立浏览器，不包含已有登录状态。此功能在本地开发和预览服务中可用，静态部署版不提供截图服务。
