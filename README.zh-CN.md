...
缺图时默认使用 EchoPod 示例（用户提供内外屏截图；应用介绍来自 https://echopod.clothpath.com/），界面及导出文件名带示例标识。关闭示例后，缺图会阻断正式导出。批量 ZIP 包含真实上传场景及由内屏左半边生成的外屏，不包含示例。

EchoPod 仅用于展示工具能力，用户上传后优先展示自己的图片。切换静态场景后调整折叠进度会保留该姿态。旧版上架套图项目导入后自动切换为展示图，并保留素材及历史设置。
