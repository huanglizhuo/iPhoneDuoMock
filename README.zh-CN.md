...
缺图时默认复刻 Apple 官网 iPhone Duo 商品图册中的官方锁屏（壁纸与 UI 合成自 https://www.apple.com/sg/iphone-duo 的视差图层），界面及导出文件名带示例标识。关闭示例后，缺图会阻断正式导出。批量 ZIP 包含真实上传场景及由内屏左半边生成的外屏，不包含示例。
...
只支持允许 iframe 嵌入的网站。静态弯折内屏使用独立分片文档，跨域交互状态不能同步；这里不模拟 Safari 或原生折叠 API。导出网页内容时，浏览器会要求授权捕获当前 Duo Studio 标签页；应用在本地把各网页视口分块转为临时截图，结束后立即停止共享，不会上传内容。动态页面应先暂停，低像素密度显示器上的锐度由浏览器合成能力决定。完整需求、行为与实现边界见 [BROWSER_MODE.md](docs/BROWSER_MODE.md)。
